...
proc `=copy`*(x: var RawTask, y: RawTask) {.error.}

proc `=destroy`*(t: var RawTask) {.inline.} =
  `=destroy`(Task(t))

proc invoke*(task: RawTask) {.borrow, gcsafe.}

template checkIsolate(scratchAssignList: seq[NimNode], procParam, scratchDotExpr: NimNode, isRaw: bool) =
  # block:
...
template addAllNode(assignParam: NimNode, procParam: NimNode, isRaw: bool) =
  let scratchDotExpr = newDotExpr(scratchIdent, formalParams[i][0])

  checkIsolate(scratchAssignList, procParam, scratchDotExpr, isRaw)
...
macro toTaskImpl(e: typed{nkCall | nkInfix | nkPrefix | nkPostfix | nkCommand | nkCallStrLit}, isRaw: static bool): Task =
    ...
    for i in 1 ..< formalParams.len:
      ...
      of nnkBracketExpr:
        if param[0].typeKind == ntyTypeDesc:
          ...
          addAllNode(seqType, seqCallNode, isRaw)
        else:
          addAllNode(param, e[i], isRaw)
...
template toTask*(e: typed{nkCall | nkInfix | nkPrefix | nkPostfix | nkCommand | nkCallStrLit}): Task =
  toTaskImpl(e, false)

template toRawTask*(e: typed{nkCall | nkInfix | nkPrefix | nkPostfix | nkCommand | nkCallStrLit}): RawTask =
  RawTask(toTaskImpl(e, true))
